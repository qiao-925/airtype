...
def deploy_config(tier_model, refine_model=None):
    ...
    if refine_model:
        cfg += (
            ...
            f'REFINE_PROMPT="你是语音输入后处理助手。对用户语音识别的原始文本做如下处理：\\n'
            f'1. 去除填充词和语气词（嗯、啊、那个、就是说、你知道的、然后、对吧）\\n'
            f'2. 去除重复词句（如\\"今天今天天气不错\\"→\\"今天天气不错\\"）\\n'
            f'3. 识别自我纠正（如\\"明天，不对，后天\\"→\\"后天\\"），只保留最终意图\\n'
            f'4. 修正明显的识别错误，补全标点符号\\n'
            f'5. 如有列表或步骤，整理为结构化文本\\n'
            f'6. 保持原意，语言简洁自然，不要添加原文没有的内容\\n'
            f'只输出修正后的文本，不要解释。"\n'
            f'REFINE_THREADS=4\n'
        )
